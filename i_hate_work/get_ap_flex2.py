...
# --------------------获取AP配置--------------------
        with tqdm(total=len(ap_list)) as pbar:
            for ap in ap_list:
                chan.send(b'show ap config general ')
                chan.send(ap)
                chan.send(b'\n')
                time.sleep(0.5)
                ...
                native_id = re.findall('.*Native ID :\.+\s(\d+).*', ap_config)
                if native_id:
                    ap_flex[ap] = native_id[0]
                else:
                    ap_flex[ap] = 'none'
                pbar.update(1)
        ...
        fname = now + r'_' + hostname + r"_AP_Native_Vlan.csv"
        with open(fname, "w", newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["AP Name", "Native Vlan"])
